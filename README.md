...
<p align="center">
  Claude·Codex(ChatGPT)·Gemini·Antigravity IDE 구독 사용량을 시스템 트레이 아이콘·데스크톱 위젯·통계 창으로
  한눈에 관리하는 데스크톱 앱입니다.
  <br />
...
>
> **Codex 역시 비공식 연동입니다.** 앱 본체와 분리된 전용 로그인 창에서 본인 ChatGPT 계정으로 로그인한 뒤
> `chatgpt.com` 세션과 비공개 구독 한도 응답을 읽습니다. 로그인 세션에서 얻은 짧은 OAuth 토큰은
> 사용량 조회에만 메모리에서 사용하고 저장하지 않습니다. OpenAI API 키, API 토큰 사용량,
> API 비용은 사용하지 않거나 추적하지 않습니다. 이 엔드포인트와 인증 쿠키 형식은 공개 계약이 아니므로
> 변경되면 로그인 또는 조회가 실패할 수 있습니다. 반드시 [OpenAI 이용약관](https://openai.com/policies/terms-of-use/)
> 을 확인하고 **본인 책임**으로 사용하십시오. "Codex"와 "ChatGPT"는 OpenAI의 상표이며 본 프로젝트는
> OpenAI와 무관합니다.
>
> **Antigravity IDE 모니터링 역시 비공식 기능입니다.** Antigravity IDE의 로컬 `language_server`
> 프로세스 API를 자동 감지하여 코딩 쿼터를 모니터링합니다. 별도의 로그인이 필요하지 않으며 Windows
> 환경만 지원합니다. Google Antigravity/AGY의 업데이트에 따라 동작이 달라질 수 있습니다.

## 기능

- **멀티 서비스 모니터링**: **Claude**, **Codex(ChatGPT 구독)**, **Gemini**, **Antigravity IDE**의 사용량을 한곳에서 모니터링합니다.
- **Codex 구독 쿼터**: ChatGPT 계정으로 로그인해 Codex의 주간 구독 한도를 위젯·통계·이력에서 확인합니다. 5시간 세션, OpenAI API 사용량·비용과 추가 모델별 한도는 범위 밖입니다.
- **격리된 Codex 로그인**: ChatGPT 로그인·세션 확인·비동기 쿠키 캡처는 전용 프로세스에서 수행하고, 반환 쿠키는 앱 본체에서 OAuth bearer로 다시 검증한 뒤 DPAPI로 저장합니다. 로그인 페이지가 멈춰도 트레이·설정·위젯은 계속 동작합니다.
- **Antigravity IDE 자동 감지**: 별도 로그인 없이 로컬에서 실행 중인 Antigravity IDE를 자동 감지해 Gemini 및 3p(Claude/GPT) 모델군의 5시간·주간 코딩 쿼터를 추적합니다(Windows 전용).
- **위젯 그리드 도킹**: 위젯 스타일 창의 "배치" 탭에서 여러 서비스 위젯을 그리드로 묶어 함께 이동·배치할 수 있습니다.
- **화면 이탈 자동 보정**: 위젯이나 도킹 그룹이 디스플레이 화면 밖으로 일부라도 나갈 경우 화면 안쪽 경계로 자동 보정하여 이동합니다.
- **트레이 아이콘**: 프로그램 이름과 버전을 깔끔하게 표시하며 테마를 따릅니다.
- **데스크톱 위젯**: 테두리 없는 항상 위(토글) 창으로 작업 표시줄에 나타나지 않습니다. 서비스별로 제공되는
  5시간·주간 사용량과 초기화까지 남은 시간을 보여 주며, 항상 위·이동 잠금 토글과 통계·설정 바로가기 버튼이
  있습니다. 컴팩트 위젯의 더보기 메뉴는 본문 위 팝오버로 열려, 메뉴를 열어도 위젯 본문과 도킹 위치가 밀리지 않습니다.
- **통계 창**(트레이 더블클릭): 전체 사용 항목, 이력 차트, 소진 예측, 초기화 일정.
- **커스텀 테마 메뉴**(트레이 우클릭): 위젯 스타일, 통계, 설정, 종료 바로가기.
...
5. 위젯 또는 설정 창에서 Claude, Codex 또는 Gemini에 로그인하면 사용량이 표시됩니다. Antigravity IDE는 실행 중이면 자동으로 연동됩니다.
...
- **서비스별 세션 분리 저장.** 로그인 창에서 얻은 Claude·Codex·Gemini 세션 쿠키는 OS 애플리케이션
  데이터 폴더의 사용자별 파일에 분리 저장됩니다. Claude는 호환성을 위해 `session.dat`, Codex는
  `session.codex.dat`, Gemini는 `session.gemini.dat`를 사용합니다. 각 쿠키는 해당 서비스
  (`claude.ai`, `chatgpt.com`, `gemini.google.com`)로의 HTTPS 요청에만 사용되며 저장소에 커밋되거나
  프로젝트로 전송되지 않습니다.
- **Windows에서는 저장 시 암호화됩니다.** 각 세션 파일은 Windows 데이터 보호 API(DPAPI, 사용자
  ...
  자체 쿠키를 보관하는 방식과 같습니다. 서비스별 로그아웃하면 해당 세션 파일이 삭제됩니다.
- **텔레메트리 없음.** 로그인·사용량 조회·업데이트 확인을 위한 서비스 및 배포 서버 통신 외에 사용
  데이터를 수집하거나 전송하지 않습니다. Antigravity IDE 데이터는 로컬 loopback API에서만 읽습니다.
...
- SessionMeter는 claude.ai·chatgpt.com·gemini.google.com의 비공식 연동과 Antigravity IDE의
  비공개 로컬 API에 의존하므로 예고 없이 동작이 바뀔 수 있습니다. 본인 계정의 세션 또는 본인 PC의
  IDE로만 사용하고, [Anthropic 약관](https://www.anthropic.com/legal/consumer-terms),
  [OpenAI 이용약관](https://openai.com/policies/terms-of-use/),
  [Google 약관](https://policies.google.com/terms)을 확인한 뒤 본인 책임으로 사용하십시오.
- 본 프로젝트는 Anthropic·OpenAI·Google·Antigravity IDE 개발사와 제휴·후원·보증 관계가 없습니다.
  "Claude"·"Anthropic"은 Anthropic, PBC의, "Codex"·"ChatGPT"는 OpenAI의, "Gemini"는 Google LLC의,
  "Antigravity"는 해당 개발사의 상표이며 호환 대상을 설명하기 위해서만 사용되었습니다.
